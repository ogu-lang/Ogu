...
(defn turtle [width height]
  (let [image (BufferedImage. width height BufferedImage/TYPE_INT_RGB)]
    (ref {:width width :height height :image image :drawing true :point [0 0] :angle 0 :color Color/white})))
...
(defn pen-color "Set pen color" [turtle color] (dosync (alter turtle merge {:color color})))

(defn right "Turn right through the angle ." [turtle degrees]
  (let [angle (:angle @turtle) heading (mod (+ angle degrees) 360)]
    (dosync (alter turtle merge {:angle heading}))))

(defn left "Turn left through the angle ." [turtle degrees]
  (right turtle (- degrees)))

(def to-awt-type {:bold Font/BOLD :plain Font/PLAIN})

(defn set-font [turtle name size type]
  (let [font (Font. name (to-awt-type type) size)]
    (dosync (alter turtle merge {:font font}))))


(defn draw-text [turtle pos text]
  (doto
    (.getGraphics (:image @turtle))
    (.setFont (:font @turtle))
    (.drawString text (first pos) (second pos))))

(defn draw-image [turtle pos img]
  (let [bmp (ImageIO/read (File. img))]
    (doto (.getGraphics (:image @turtle))
      (.drawImage bmp (first pos) (second pos) (.getWidth bmp) (.getHeight bmp) nil))))
...
(defn forward "Move forward by turtle steps." [turtle steps]
  (let [p1 (:point @turtle)
        angle (:angle @turtle)
...
(defn back "Move backward by turtle steps." [turtle steps]
  (forward turtle (- steps)))

(defn go "Move to the given point." [turtle x y]
  (let [p1 (:point @turtle) p2 [x y]]
    (if (true? (:drawing @turtle)) (draw-line turtle p1 p2))
    (dosync (alter turtle merge {:point p2}))))

(defn toward "Turn to face the given point." [turtle x y]
  (let [p (:point @turtle)
        polar (Math/atan2 (- (first p) x) (- (second p) y))
        angle (mod (/ polar deg) 360)]
    (dosync (alter turtle merge {:angle angle}))))

(defn distance "Return the distance between the turtle and the given point." [turtle x y]
  (let [p (:point @turtle)] (Math/sqrt (+ (Math/pow (- (first p) x) 2) (Math/pow (- (second p) y) 2)))))
